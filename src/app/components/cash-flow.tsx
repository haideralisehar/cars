...
import { getCashFlows } from '@/app/api/CashFlow/cash';
...
interface IncomingTransactionItem {
  id?: string;
  date: string;
  amount: number;
  source: TransactionSource;
  reference: string;
  paymentMode: 'Cash' | 'Transfer' | 'N/A';
  allocationStatus: AllocationStatus;
}

interface OutgoingTransactionItem {
  id?: string;
  date: string;
  amount: number;
  usageType: string;
...
interface CashFlowData {
  ...
  incoming: IncomingTransactionItem[];
  outgoing: OutgoingTransactionItem[];
}

export function CashFlow({ userRole }: CashFlowProps) {
  const [filterType, setFilterType] = useState<'All' | TransactionSource>('All');
  const [allocationPanelOpen, setAllocationPanelOpen] = useState(false);
  const [selectedTransaction, setSelectedTransaction] = useState<IncomingTransactionItem | null>(null);
  const [historyModalOpen, setHistoryModalOpen] = useState(false);
  const [traceModalOpen, setTraceModalOpen] = useState(false);
  ...
  // Fetch cash flow data on component mount
  useEffect(() => {
    fetchCashFlowData();
  }, []);
  ...
  const handleOpenAllocationPanel = (transaction: IncomingTransactionItem) => {
    setSelectedTransaction(transaction);
    setAllocationRows([
      {
        id: '1',
  ...
  const filteredIncoming = cashFlowData.incoming.filter(
    (t) => filterType === 'All' || t.source === filterType
  );

  // Pagination calculations for incoming
  const totalIncomingPages = Math.ceil(filteredIncoming.length / itemsPerPage);
  const startIncomingIndex = (incomingCurrentPage - 1) * itemsPerPage;
  const endIncomingIndex = startIncomingIndex + itemsPerPage;
  const currentIncomingData = filteredIncoming.slice(startIncomingIndex, endIncomingIndex);

  // Pagination calculations for outgoing
  const totalOutgoingPages = Math.ceil(cashFlowData.outgoing.length / itemsPerPage);
  const startOutgoingIndex = (outgoingCurrentPage - 1) * itemsPerPage;
  const endOutgoingIndex = startOutgoingIndex + itemsPerPage;
  const currentOutgoingData = cashFlowData.outgoing.slice(startOutgoingIndex, endOutgoingIndex);
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card className="bg-card border-border">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-foreground">
                {cashFlowData.summary.companyBalance.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-card border-border">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-foreground">
                {cashFlowData.summary.investorBalance.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-card border-border">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-foreground">
                {cashFlowData.summary.vatPayable.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-card border-border border-destructive/20">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-destructive">
                {cashFlowData.summary.unallocated.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </div>
            </CardContent>
          </Card>
        </div>
              ...
              <CardContent>
                <div className="overflow-x-auto">
                  <table className="w-full">
                    ...
                    <tbody>
                      {currentIncomingData.map((transaction, index) => (
                ...
                {filteredIncoming.length > itemsPerPage && (
                  <div className="flex items-center justify-between mt-6 pt-4 border-t border-border">
                    <div className="text-sm text-muted-foreground">
                      Showing {startIncomingIndex + 1} to {Math.min(endIncomingIndex, filteredIncoming.length)} of {filteredIncoming.length} entries
                    </div>
                    <div className="flex gap-2">
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => setIncomingCurrentPage(prev => Math.max(1, prev - 1))}
                      ...
                      <div className="flex gap-1">
                        {Array.from({ length: totalIncomingPages }, (_, i) => i + 1).map((page) => (
                          <Button
                            key={page}
                            variant={incomingCurrentPage === page ? "default" : "outline"}
                            size="sm"
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => setIncomingCurrentPage(prev => Math.min(totalIncomingPages, prev + 1))}
                        disabled={incomingCurrentPage === totalIncomingPages}
                        className="gap-1"
                      >
                        Next
            ...
            <Card className="bg-card border-border">
              ...
              <CardContent>
                ...
                {cashFlowData.outgoing.length > itemsPerPage && (
                  <div className="flex items-center justify-between mt-6 pt-4 border-t border-border">
                    <div className="text-sm text-muted-foreground">
                      Showing {startOutgoingIndex + 1} to {Math.min(endOutgoingIndex, cashFlowData.outgoing.length)} of {cashFlowData.outgoing.length} entries
                    </div>
                    <div className="flex gap-2">
                      <Button
                        variant="outline"
                      ...
                      <div className="flex gap-1">
                        {Array.from({ length: totalOutgoingPages }, (_, i) => i + 1).map((page) => (
                          <Button
                            key={page}
                            variant={outgoingCurrentPage === page ? "default" : "outline"}
                            size="sm"
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => setOutgoingCurrentPage(prev => Math.min(totalOutgoingPages, prev + 1))}
                        disabled={outgoingCurrentPage === totalOutgoingPages}
                        className="gap-1"
                      >
                        Next
                        <ChevronRight className="h-4 w-4" />
                    ...
                    <CardContent className="p-4 space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <Select
                            value={row.destination}
                            onValueChange={(value) =>
                              handleUpdateAllocationRow(row.id, 'destination', value as Destination)
                            }
                          >
                            <SelectTrigger className="bg-input-background">
                              <SelectValue />
                      ...
                      <div className="border-t border-border pt-3">
                        ...
                        <Select
                          value={row.usageType}
                          onValueChange={(value) =>
                            handleUpdateAllocationRow(row.id, 'usageType', value as UsageType)
                          }
                        >
                          <SelectTrigger className="bg-input-background">
                            <SelectValue />
