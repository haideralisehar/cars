                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                          ...
                          <div className="flex justify-between items-center">
                            ...
                            {carData?.lease && carData?.lease?.payments ?
                              <span className="font-medium text-green-500">
                                BHD {Number(
                                  (
                                    carData.lease.payments.reduce((sum, payment) => sum + (payment.amount || 0), 0) +
                                    (carData?.lease?.advancePayment || 0)
                                  ).toFixed(2)
                                )}
                              </span>
                        ...
                        <div className="p-6 bg-primary/5 rounded-xl border border-primary/10 flex flex-col justify-center items-center">
                          ...
                          {carData?.sale ?
                            <span className="text-4xl font-black text-primary">
                              BHD {(
                                ((carData?.sale?.sellingPrice || 0) -
                                  (
                                    (carData?.financialDetails?.buyingPrice || 0) +
                                    (carData?.moneyRecords?.reduce(
                                      (total, record) => total + (record.payableAmount || 0),
                                      0
                                    ) || 0)
                                  )
                                ) -
                                ...
                                (
                                  carData?.lease?.payments
                                    ? carData.lease.payments.reduce(
              ...
              <CardContent>
                <div className="space-y-4">
                  {timelineEvents.map((event) => (
                    <div key={event.id} className="flex gap-4">
                      <div className="flex flex-col items-center">
                        <div className={`w-3 h-3 rounded-full ${event.category === 'expense' ? 'bg-red-500' :
